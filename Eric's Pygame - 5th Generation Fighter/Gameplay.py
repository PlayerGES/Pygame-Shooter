...
print("Welcome to the advanced fighter game")
print("In this game you'll take on the role of pilot and destroy some enemies")
print("Your task is to destroy as much enemies as possible from your airport to the green beacon")
print("After you reach the beacon, you win the round")
print("You can get money through winning or destroying enemies")
print("The money can then be used to buy upgrades in the store")
print("WASD to move")
print("Up and down arrow button to increase or decrease speed")
print("Left mouse to shoot")
print("Good luck flying :)")
time.sleep(5)
red = (255, 0, 0)
yellow = (225, 225, 0)
green = (0, 200, 0)
...
machineGunSound = pygame.mixer.Sound(os.path.join('Vulcan Gun Sound.mp3'))
machineGunSound.set_volume(0.1)
tankGunSound = pygame.mixer.Sound(os.path.join('Tank Firing.mp3'))
missileSound = pygame.mixer.Sound(os.path.join('Missile Launch.mp3'))
missileSound.set_volume(0.5)
tankDrivingSound = pygame.mixer.Sound(os.path.join('Tank Driving.mp3'))
tankDrivingSound.set_volume(0.1)
...
def individualBullet(startingPointX, startingPointY, endingPointX, endingPointY, firing):
    ...
    if firing:
        if playerFighterType == F35:
            ...
            color = green
        return pygame.draw.line(screen, color, (startingPointX, startingPointY), (endingPointX, endingPointY), 2)
    else:
...
def shootingFunction(playerX, playerY):
    ...
    if setCoordinate:
        ...
        firstTime = True
        bulletCount -= 1
        pygame.mixer.Sound.play(machineGunSound)
...
def enemyTankShootingFunction(enemyX, enemyY):
    ...
    if enemy2SetCoordinate:
        ...
        pygame.mixer.Sound.play(tankGunSound)
    if enemy2Repetition > 0:
        ...
        if enemy2FinalLocation_x >= enemyX and enemy2FinalLocation_y >= enemyY:
            ...
            endY = enemyY + length_y + length_y * enemy2Times
        if enemy2Times == 1:
            bullet = pygame.draw.line(screen, red, (enemyX, enemyY), (endX, endY), 3)
            shootingEffect = pygame.transform.scale(pygame.image.load("ShootingEffect2.png"), (16, 10))
            screen.blit(shootingEffect, (enemyX, enemyY - 5))
        else:
            if enemy2ContinueShooting:
                bullet = pygame.draw.line(screen, red, (startX, startY), (endX, endY), 3)
            else:
...
def enemyMissileShootingFunction(enemyX, enemyY, targetX, targetY):
    ...
    if enemy3Timer != enemy3Time and not enemy3SetCoordinate:
        ...
        if enemy3ContinueShooting:
            if not lockedOn:
                value = enemyX - targetX
                if value == 0:
                    value = 0.00001
                if targetX < missile_x and targetY < missile_y:
                    degrees = 180 - (57.2958 * (math.atan(abs((enemyY - targetY) / value))))
                elif targetX > missile_x and targetY < missile_y:
                    degrees = 57.2958 * (math.atan(abs((enemyY - targetY) / value)))
                elif targetX > missile_x and targetY > missile_y:
                    degrees = 180 + (57.2958 * (math.atan(abs((enemyY - targetY) / value))))
                else:
                    degrees = 360 - (57.2958 * (math.atan(abs((enemyY - targetY) / value))))
            else:
                degrees = 0
...
while True:
    ...
    if selectingFighter:
        ...
        upgradeHealthText = myFighterExplanationFont.render(
            "Upgrade Health (" + str(int(additionalHealth / 500)) + ") for 10000 dollars", False, black)
        upgradeDamageText = myFighterExplanationFont.render(
            "Upgrade Damage (" + str(int(additionalDamage / 50)) + ") for 10000 dollars", False, black)
        upgradeMinSpeedText = myFighterExplanationFont.render(
            "Upgrade Hover (" + str(int(-additionalLowSpeed * 2)) + ") for 10000 dollars", False, black)
        upgradeMaxSpeedText = myFighterExplanationFont.render(
            "Upgrade Speed (" + str(int(additionalHighSpeed * 2)) + ") for 10000 dollars", False, black)
    ...
    # Process of displaying fighter's background information
        ...
        if enemy3Health <= 0:
            ...
            enemy3Health = enemy3MaxHealth
            enemy3_x = random.randint(2000, 2500)
        if enemy2Health > 0 and tankDrivingSoundTimer > 1:
            # pygame.mixer.Sound.play(tankDrivingSound)
            tankDrivingSoundTimer = 0
